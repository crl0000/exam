...
var slider_plugin = (function() {
  ...
  fifi_slider.prototype.buildDots = function () {
    ...
    for (var i = 0; i < dots.totalSlides; i++) {
      var dot = document.createElement('li');
      dot.setAttribute('data-slide', i + 1);
      dots.def.dotsWrapper.appendChild(dot);
    }

    dots.def.dotsWrapper.addEventListener('click', function (e) {
      if (e.target && e.target.nodeName == "LI") {
        dots.curSlide = e.target.getAttribute('data-slide');
        dots.gotoSlide();
      }
    }, false);
  }
